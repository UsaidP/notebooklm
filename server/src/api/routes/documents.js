  ...
  asyncHandler(async (req, res) => {
    ...
    for (const file of files) {
      try {
        ...
        console.log(
          `[Upload] Uploaded ${file.originalname} to Appwrite: ${appwriteFile.$id}`,
        );
        ...
        try {
          ...
          console.log(
            `[Upload] Queued document for processing: ${document.id}`,
          );
        } catch (queueError) {
          console.warn(
            `[Upload] Queue unavailable for ${document.id}: ${queueError.message}`,
          );
          console.warn(
            "[Upload] Document will remain in QUEUED status until manually processed",
          );
        }
        ...
        console.error(
          `[Upload] Error processing ${file.originalname}:`,
          error.message,
        );
        uploadedDocuments.push({
          name: file.originalname,
    ...
    res.status(201).json({
      ...
      summary: {
        total: files.length,
        succeeded: uploadedDocuments.filter((d) => d.status === "QUEUED")
          .length,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      await addDocumentJob({
        documentId: document.id,
        userId: document.notebook.userId,
      ...
      console.warn(`[Retry] Queue unavailable for ${document.id}: ${queueError.message}`);
    }
